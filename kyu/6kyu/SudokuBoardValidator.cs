...
public static partial class kyu6
{
    ...
    public static bool Validate(int[][] board)
    {
        ...
        List<int> appearances = new List<int>(9);
        ...
        for (var i = 0; i < 9; i++)
        {
            for (var k = 0; k < 9; k++)
            {
                if (board[i][k] == 0)
                    return false;
                else if (!appearances.Contains(board[i][k]))
                    appearances.Add(board[i][k]);
                else
                    return false;
            }
            appearances.Clear();
        ...
        {
            for (var k = curCol; k < maxCol; k++)
            {
                if (appearances.Contains(board[i][k]))
                    return false;
                if (!appearances.Contains(board[i][k]))
                    appearances.Add(board[i][k]);
                if (k == 8 && i == 8)
